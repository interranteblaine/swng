// The canonical round designation (accounts-only identity spec §5): a round is referred to
// everywhere — the home list, the archive page, the watch page, the join link's framing — as
// COURSE + DATE, "Casa Verde GC · Sat, Jul 12", with the tee time appended ("· 7:58a") only when
// two rounds share course AND day. A PURE function of facts the round already records (the frozen
// course card's name + the created-at instant): nothing is stored, no name field, no tags.
//
...
export interface RoundDesignation {
  ...
  // OPTIONAL by construction (spec §5 + the no-migration rule): old projection rows written
  // before this feature carry no created-at, tolerated on read as absent. A round with no
  // created-at simply renders as its bare course name — no date segment, no placeholder.
...
const dayOf = (createdAt: number, timeZone?: string): string => {
  const parts = new Intl.DateTimeFormat("en-US", { weekday: "short", month: "short", day: "numeric", timeZone }).formatToParts(new Date(createdAt));
...
const timeOf = (createdAt: number, timeZone?: string): string => {
  const parts = new Intl.DateTimeFormat("en-US", { hour: "numeric", minute: "2-digit", hour12: true, timeZone }).formatToParts(new Date(createdAt));
...
export const roundLabel = ({ courseName, createdAt }: RoundDesignation, { withTime, timeZone }: RoundLabelOptions = {}): string => {
  if (createdAt === undefined) return courseName;
  const base = `${courseName} · ${dayOf(createdAt, timeZone)}`;
  return withTime ? `${base} · ${timeOf(createdAt, timeZone)}` : base;
};
...
// (local by default — see the module doc's zone-basis requirement). `undefined` when there is no
// created-at to place the round on a day — such a round has no day and so can never collide, and
// always renders as its bare course name. The day is appended as `year-month-day` (zero-padded)
// so two rounds at the same course collide iff they fall on the same calendar day in that zone.
export const roundDayKey = ({ courseName, createdAt }: RoundDesignation, { timeZone }: RoundDayKeyOptions = {}): string | undefined => {
