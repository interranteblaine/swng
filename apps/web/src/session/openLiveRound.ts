...
// session. `joinCode: ""` because a re-mint outside the join flow carries no join code (nothing
// downstream needs it — this device is entering a round its own account golfer already sits
// in). A byte-identical move of HomePage's own click-handler body, so BOTH callers — HomePage's
// "Your rounds" list and RoundRecordPage's own live-check branch (§7 step 2) — share ONE
// re-mint implementation rather than two. Error handling (ApiError branching into human copy)
// stays at each call site — that's caller-specific UI, not part of the re-mint itself.
export async function openLiveRound(id: RoundId, deps: OpenLiveRoundDeps): Promise<void> {
  ...
  credentialStore.save(response.roundId, { token: response.token, golferId: response.golferId, name: deps.golferName, joinCode: "" });
