...
//      longer runs. Asserting it from `pnpm test` — a different leg of `pnpm validate` — means
//      removing the pin takes two edits in two files instead of one deletion.
...
//      served field fails the build until someone classifies it.
//
...
/** Every field declared `z.number(...)` anywhere in the wire schemas. */
