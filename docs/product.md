...
- **The ledger.** All-time and per-season records, filled automatically: a per-member scoreboard (rounds, best 18, net per 18, index with its change over the season) built from every round a member finishes — together or apart — plus head-to-head match results between any two members (*you're 7–6 lifetime against Dave, and swng reminds you both on the first tee*), partner records (*you and Mike are 9–2 in four-ball*), skins won, and points leaderboards from the rounds members actually shared.
