...
- **Design decisions fixed by this plan** (do not relitigate):
  ...
  - **Server-authored vs client-authored events.** `round-created`/`participant-joined`/`game-added`/`round-started`/`round-finalized` are constructed server-side: `opId` from `IdGenerator`, `hlc` from a per-invocation HLC source that is **strictly monotonic within a batch** — `wallMs = clock.now()`, and `counter` increments when `wallMs` hasn't advanced since the previous stamp (amended during execution: the original `counter: 0` spec let StartRound's three same-millisecond events collide on hlc, leaving the status register to a random-opId tiebreak — rounds went live only ~50% of the time; caught when the E2E gate was re-run past its lucky streak), `deviceId = deviceId("server")`, `authorId` = the commanding golfer (host for creation events, joiner for their join, finalizer for finalize). `score-recorded` is client-authored: the client supplies `opId`, `hlc`, `golferId` (subject), `hole`, `result`; the server stamps `authorId` from the verified token — a client can never claim another author.
