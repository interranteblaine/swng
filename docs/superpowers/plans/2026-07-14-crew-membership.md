...
- **Expiry:** `CREW_INVITE_TTL_MS = 7 * 24 * 60 * 60 * 1000`, applied in `mintCrewInvite` from the `Clock` port (`clock.now() + CREW_INVITE_TTL_MS`). `verify()` must reject an expired crew-invite (confirm/extend the issuer's existing participant-exp handling — the M4-era participant tokens carry `exp`). (Superseded at C-T1 review: verify() deliberately does NOT expiry-gate crew-invite — a verify-level rejection collapses to `undefined` and would make `crew-invite-expired` wire-indistinguishable from `crew-invite-invalid`, violating the spec's binding two-distinct-codes requirement; expiry is enforced in the two use cases instead, with the complete verify() call-site set traced to prove no path where an expired invite grants anything.)
