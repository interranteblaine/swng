...
| T5 | — | `apps/web/src/courses/AddCoursePage.tsx` + `EditCoursePage.tsx` + `CoursePage.tsx` + `CourseSummaryCard.tsx` (+tests), `apps/web/src/routes/{CreateRoundPage,JoinRoundPage,ProfilePage}.tsx` (+tests), `packages/contracts/src/courses.ts` (peek `par` + `newTeeInputSchema` write-widening), `packages/application/src/rounds/peekRound.ts` |
...
- [ ] **Step 1: contracts — peek `par` + widen the course-write input.** Two schema changes in `packages/contracts/src/courses.ts` (+ `peekRound.ts`):
  - **Peek `par`.** `PeekRoundResponse.teeSets` gains `par` (required int). NOTE: T1 already made peek's `rating`/`slope` optional (the necessary blast-radius of `TeeSet.rating/slope` going optional) and `peekRound.ts` already conditional-spreads them — so this step ONLY adds `par`. `peekRound.ts` maps `par: tee.holes.reduce((s,h)=>s+h.par,0)`.
  - **Write path (the gap that lets an unrated course be SUBMITTED).** `newTeeInputSchema` (courses.ts:~27, the create/supersede tee input, inherited by `continuingTeeInputSchema`) still REQUIRES rating+slope. Widen its `rating`/`slope` to `.optional()` — mirroring round.ts's `teeSetSchema` — so an unrated tee can be POSTed at all. The domain `validateCard` (T1) already enforces the pairing + bounds server-side and rejects one-of-two with `rating-slope-paired`, so the wire schema stays purely structural (both `.optional()`, no `.refine`). Without this, entry (Step 2) has no server shape to submit to and unrated-course creation is impossible.
- [ ] **Step 2: entry/display.** AddCoursePage + EditCoursePage: rating and slope inputs become optional — helper text "No course rating on the card? Leave these blank." `canSubmit` no longer requires them; parse them only when non-blank; a value in exactly one of the two shows the `rating-slope-paired` server error on the pair. **Rework EditCoursePage's `carryOver`** (T1 left a placeholder that THROWS `tee-unrated` via an `isRated` guard, valid only while unrated cards can't exist): now that the write schema accepts unrated tees, an existing unrated card's *other* tees must round-trip untouched through a supersede — so pass each carried-over tee's rating/slope through conditionally (spread-omit when absent, per the no-explicit-undefined constraint), never guarding or asserting. Display everywhere a tee's numbers render — CoursePage tee `<option>`/summary, CourseSummaryCard's `<select>` and attribution, both create/join tee pickers — use a shared helper:
