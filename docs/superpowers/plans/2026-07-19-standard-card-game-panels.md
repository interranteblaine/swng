...
- [ ] **Step 2: Failing web tests:**
  ...
  - GamePanel: header order title → treatment line → blurb → strokes line → note; stroke-play NET treatment line `Net — 95% handicap (standard)`; GROSS `Gross — raw scores, no strokes` and NO strokes line; stableford/skins strokes line present; singles note verbatim; stroke-play rows sorted by vs-par then thru (fixture: +2 thru 17 ranks ABOVE E thru 0 — the exact mid-round case the sort ruling fixes); all-zero strokes fixture reads `No strokes — everyone plays off 0.`
  - AddGameForm: fourball selection shows its note; singles note now comes from the shared source (string unchanged).
  Run — FAIL.

- [ ] **Step 3: Implement.** GamePanel = GameSheet's bodies unchanged; wrapper becomes `<section role="region" aria-label={`${title} standings`} className="flex flex-col gap-3 rounded-lg bg-slate-900 p-4">` (no fixed positioning, no ✕); header per spec §2c (treatment line logic: `game.kind === "stroke-play" ? (game.scoring === "net" ? `Net — ${allowancePhrase("stroke-play", config?.allowance)}` : "Gross — raw scores, no strokes") : config && allowancePhrase(config.kind, config.allowance)`); `strokesSummary` for every kind except gross stroke play; `strokesNote(game.kind)` line when defined; End footer:
