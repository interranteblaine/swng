...
- [ ] **Step 5:** `AWS_PROFILE=swng node scripts/migrateProdStrokes.mjs --stage prod --write --after-deploy`. Confirm the backup path is printed and the file exists before proceeding. Expect 15 records written.

  `--after-deploy` is required by the script and is an assertion that Step 4 has actually completed. Migrating first is not a smaller mistake — the old projector consumes the snapshot writes and stamps the old shape back onto the record lines, and re-running afterwards will NOT repair it (the transform is idempotent, so the second run writes nothing and never re-fires the stream). If that state is ever reached, the recovery is `rebuildProjections`, which re-projects every snapshot.
...
- [ ] **Step 7:** `AWS_PROFILE=swng node scripts/checkProdParses.mjs --stage prod` → **exit 0, zero failures**, and the printed strokes reading 21/36, 13/18/10, 20/0/36, 31.
  ...
  The 12 events pass immediately. The 8 record lines pass only once the projector has consumed the snapshot writes, so a first run failing on record lines alone is projector lag, not a defect — wait and re-run. A first run failing on any *event* or *archive* is a real failure and stops here.
