...
> Status: **approved** (2026-07-29, owner design session). Supersedes the handicap model in
...
### 2b. Strokes are derived, never stored

Strokes fall out of the fold across the whole roster, the way scores fold into standings. One
rule:

> **Strokes are the difference from the lowest in the field.** The anchor is the lowest stated
> `normally-shoots` on the roster. A player who stated a normal score gets `theirs − anchor`. A
> player who stated strokes gets exactly what they said.

**The field is whatever you are looking at.** The card's field is the round's roster; a game's
field is that game's players. Same rule, no exceptions, no percentages.
...
The three cases this must produce:

| roster | result |
|---|---|
| `+30` and `+10` | anchor 10 → **20 and 0** |
| Blaine flips to `strokes: 18` | **18 and 0**; his `+30` is untouched and still true |
| `+30` and `strokes: 0` | anchor 30 → **0 and 0** — correct, not a failure: strokes cannot be allocated when only one person's level is known, and the fix is visible on screen |
...
- The join field is **pre-filled** from your record: the average of `score − par` over your last
  10 finished rounds, a 9-hole round contributing its figure doubled.
- Fewer than 3 finished rounds: pre-filled from the last number you stated. First round ever:
  blank, you type it.
...
- **Picked up** — you stopped. There is no number and nobody pretends otherwise. A round
  containing one does not feed the average. It still shows in your history and still counts for
  `Best`.
...
**No game re-derives strokes.** The allowance table is deleted; each game applies §2b's one rule
to its own field and states the result in words in its panel.
...
the card's stroke index (`courseHandicapAllocation`, renamed). Chips still never touch the grid.
...
**The trend chart plots your average over time** — one line, `score − par` as of each round,
replacing the two-line index chart. `+31` in April, `+26` in July.
...
- **Average** — level.
- **Spread** — standard deviation of those numbers, gated at 5 rounds. Not a stat for its own
...
**Comparing two members who never played together is the difference of their averages**, which is
also the strokes between them:

> **If you played tomorrow, Blaine gets 16.**

Same subtraction the round does at join. It is the product's core promise stated between two
people who have not teed off.
...
`handicap/present.ts` **survives** — a player can shoot under par, so `formatHandicapIndex`
(`-1.2 → "+1.2"`), `formatCourseHandicap`, `strokeGrant`, and the whole-tree grep gate that
forbids a bare signed render all still hold. Only the pipeline behind the numbers is deleted, not
golf's plus convention.

## 8. Wire, storage, and beta

- `Participant.courseHandicap: number` → `Participant.basis: StrokeBasis`, plus a
  fold-derived `strokes: number` on `RosterEntry` (derived at fold time, stored nowhere). Every
  reader that used `courseHandicap` reads `strokes`.
...
- `JoinRoundRequest.courseHandicap` → `basis`. `PeekRound` keeps `par`/`holes`, drops nothing.
...
- `GolferMetrics`: `{ average?, spread?, best, milestones, typicalEighteen, averageHistory }`.
- `HoleResult`'s `conceded` arm gains required `strokes`.
...
`scoring/present.ts` keeps `strokesSummary`/`strokesNote`, rewritten to name the field instead of
a percentage: *"Strokes are the difference between you two"*, *"Everyone plays off the lowest of
the four."* `strokePlayTreatment` becomes `Net — uses the strokes on the card` /
