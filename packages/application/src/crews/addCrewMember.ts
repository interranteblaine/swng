...
// path that minted a fresh unclaimed ghost from a `name` is GONE — a crew member must already
// carry a bound sub (a real account). Onboarding a person without an account stays a ROUND
// concern (play as ghost → claim in-round → account → then join a crew by code, or get added
// here). The name comes from the golfer's own record, never the wire.
export const addCrewMember =
  ...
  async (claims: AccountClaims, id: CrewId, command: AddCrewMemberRequest): Promise<AddCrewMemberResponse> => {
    ...
    // ghost" means (createDynamoGolferStore's bindSub writes it). A missing row or an unclaimed
    // ghost both fail the same way: ghost-not-addable (claim the ghost first, then add).
