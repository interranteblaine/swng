...
export const reduceRound = (events: readonly RoundEvent[]): RoundState => {
  ...
  //
  //    Strokes corrections layer the same way: `participant-strokes-set` overrides the seat's
  //    `strokes` iff strictly later than the latest join, touching neither presence nor any other
  //    seat field.
  ...
  for (const event of deduped) {
    ...
    } else if (event.kind === "participant-strokes-set") {
      const existing = strokesSetsByGolfer.get(event.golferId);
      if (!existing || compareHlc(event.hlc, existing.hlc) > 0) strokesSetsByGolfer.set(event.golferId, { strokes: event.strokes, hlc: event.hlc });
    }
  }
  // The roster IS what the log asserts about each seat, in join order — there is no second pass.
  // Nothing here reads another seat's number (spec 2026-07-30 §2), which is why setting one
  // player's strokes can never move anyone else's.
